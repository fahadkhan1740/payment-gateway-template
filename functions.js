...
EcwidApp.init({
  app_id: 'hesabe-payment-dev', // use your application namespace
  autoloadedflag: true,
  autoheight: true
})
...
function goBack() {
  ...
  for (
    i = 0;
    i < document.querySelectorAll('.separate-editing-page').length;
    i++
  ) {
    document.querySelectorAll('.separate-editing-page')[i].style.display =
      'none'
  }
}
    ...
    if (fieldVisibility !== undefined) {
      if (allInputs[i].tagName == 'INPUT') {
        if (allInputs[i].type == 'checkbox' || allInputs[i].type == 'radio') {
          applicationConfig[fieldVisibility][
            allInputs[i].dataset.name
          ] = String(allInputs[i].checked)
        }
        if (
          allInputs[i].type == 'text' ||
          allInputs[i].type == 'number' ||
          allInputs[i].type == 'date'
        ) {
          applicationConfig[fieldVisibility][allInputs[i].dataset.name] =
            allInputs[i].value
        }
      }
      if (
        allInputs[i].tagName == 'SELECT' ||
        allInputs[i].tagName == 'TEXTAREA'
      ) {
        applicationConfig[fieldVisibility][allInputs[i].dataset.name] =
          allInputs[i].value
      }
